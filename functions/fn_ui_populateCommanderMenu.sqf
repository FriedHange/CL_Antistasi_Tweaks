...
private _fnc_checkIfAppropriateLocation = {
	...
	if (!isNil "traderMarker" && {
		theBoss distance2D (getMarkerPos traderMarker) < 50
	}) exitWith {
		true
	};
	false;
...
if (call _fnc_checkIfAppropriateLocation) then {
	...
	if (areRivalsDiscovered) then {
		...
		_firstPart resize 3; // Keep: Abilities, Attack Planning, Garrison

		private _secondPart = +menuSliderArray;
		reverse _secondPart;
		_secondPart resize 4; // Keep: Game Info, Game Options, Environment, HQ
		reverse _secondPart;
