...
while {!_done} do {
    ...
    if (_isMortar && {_mortarDeployCount >= _maxMortarDeployments || {!_mortarAmmoUnlimited && {_mortarRoundsFired >= _maxMortarRoundsCfg}}}) exitWith {
        ...
        { [_x, _targetPos] remoteExec ["A3A_fnc_planning_localDoMove", owner _x]; } forEach (units _group);
        _done = true;
    };
    if (_done) exitWith {};
    ...
    if (_isMortar && _firstDeployment) then {
        ...
        _group setCombatMode (if (_isMortar) then {"YELLOW"} else {"RED"});
    ...
    while {alive (leader _group) && {count (units _group) > 0} && {alive _staticVeh} && {!_relocate}} do {
        ...
        private _fnc_isOnObjective = {
            ...
            if (_p distance2D _liveObjPos < _aoRadius) exitWith { true };
            if (_haveBattlePoint2 && {_p distance2D _frontPos < 300}) exitWith { true };
            false
        };

        private _mortarCandidates = if (_isMortar) then {
            allUnits select {
                alive _x && {side _x in [Occupants, Invaders]} && {_x distance2D _curPos < _maxDist} && {[_x] call _fnc_isOnObjective}
            };
        } else { [] };
        ...
        private _targetsInRange = if (_isMortar) then {
            ...
            { alive _x && {side _x in [Occupants, Invaders]} && {_x distance2D _curPos < _maxDist} } count allUnits
        };
