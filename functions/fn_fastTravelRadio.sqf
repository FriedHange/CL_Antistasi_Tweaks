/*
    ...
    Overridden fast travel handler supporting customizable fast travel travel times, instant travel,
    and full compatibility with Antistasi Ultimate fast travel settings.
*/
params [
    ["_quickMarker", "", [""]]
];
...
if (isNil "limitedFT") then { limitedFT = 0; };
...
if (_positionTel distance getMarkerPos _base < 500) then {
	...
	// Apply customizable speed multiplier
	private _speedMult = missionNamespace getVariable ["A3A_tweak_fastTravelSpeedMultiplier", 1];
	if (_speedMult >= 9999) then {
		_distanceX = 0;
	} else {
		_distanceX = (round (_distanceX / _speedMult)) max 1;
	};
	...
	if (!isNil "SCRT_fnc_ui_toggleMenuBlur") then { ['off'] call SCRT_fnc_ui_toggleMenuBlur; };
} else {
	[localize "STR_A3A_Dialogs_fast_travel_header", localize "STR_A3A_Dialogs_fast_travel_missclick"] call SCRT_fnc_misc_deniedHint;
};
