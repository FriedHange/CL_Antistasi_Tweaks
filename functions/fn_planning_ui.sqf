...
A3A_fnc_planning_getSquadDetails = {
    ...
    switch (_squadIndex) do {
        case 0: {
            _squadType = A3A_faction_reb get "groupSquad";
            _idFormat = "Squd-";
            _displayName = "Infantry Squad";
        };
        case 1: {
            _squadType = A3A_faction_reb get "groupMedium";
            _idFormat = "Tm-";
            _displayName = "Infantry Team";
        };
        case 2: {
            _squadType = A3A_faction_reb get "groupAT";
            _idFormat = "AT-";
        ...
        case 10: {
            ...
            _idFormat = "Trk-";
            _special = "VehicleSquad";
            _displayName = "Transport Truck";
            _vehType = A3A_planning_cachedVehicles getOrDefault ["TRUCK", ""];
        };
        case 11: {
            _squadType = [_crewType, _crewType];
            _idFormat = "APC-";
        ...
        case 8: {
            ...
            _idFormat = "Crew-";
            _special = "GarageCrew";
            _displayName = "Vehicle Crew Squad";
            _vehType = _assignedGarageVeh; // Custom vehicle assigned from HQ garage
        };
    };
    ...
    if (_squadIndex in [0, 1, 2, 3, 4, 5]) then {
        {
        ...
        if (_squadIndex == 4 && {count (A3A_faction_reb getOrDefault ["groupMG", []]) == 0}) then {
            private _staticMG = (A3A_faction_reb getOrDefault ["staticMGs", [""]]) # 0;
            private _mgCost = if (_staticMG != "") then { [_staticMG] call A3A_fnc_vehiclePrice } else { 200 };
            _money = _money + _mgCost;
        };
        if (_squadIndex == 5 && {count (A3A_faction_reb getOrDefault ["groupMortar", []]) == 0}) then {
            private _staticMortar = (A3A_faction_reb getOrDefault ["staticMortars", [""]]) # 0;
            private _mortarCost = if (_staticMortar != "") then { [_staticMortar] call A3A_fnc_vehiclePrice } else { 350 };
        ...
        switch (_squadIndex) do {
            case 6: {
                // AT Car
                private _vehCost = if (_vehType != "") then { [_vehType] call A3A_fnc_vehiclePrice } else { 400 };
                _money = (2 * _crewCost) + _vehCost;
                _hr = 2;
            };
            case 7: {
                // AA Truck (build cost includes the static AA price and the truck price)
                private _staticAA = (A3A_faction_reb getOrDefault ["staticAA", [""]]) # 0;
                private _staticCost = if (_staticAA != "") then { [_staticAA] call A3A_fnc_vehiclePrice } else { 300 };
                private _truckCost = if (_vehType != "") then { [_vehType] call A3A_fnc_vehiclePrice } else { 150 };
                _money = (2 * _crewCost) + _staticCost + _truckCost;
                _hr = 2;
            };
            case 8: {
                // Vehicle Crew Squad (vehicle is from the garage, so only crew cost is paid)
                _money = 2 * _crewCost;
                _hr = 2;
            };
        };
...
if (isNull _controlsGroup) then {
    ...
    _ctrlSquadCombo ctrlSetPosition [1 * _uW, 9 * _uH, 22 * _uW, 6.5 * _uH];
    ...
    _ctrlSquadCombo ctrlAddEventHandler ["LBSelChanged", {
        ...
        private _dataId = _control lbData _selectedIndex;
        if (_dataId != "") then {
            A3A_planning_selectedSquadIndex = parseNumber _dataId;
        };

        // Show/Hide Garage Selection depending on if Vehicle Crew is selected
    ...
    _ctrlSquadEntryCombo ctrlSetPosition [1 * _uW, 16 * _uH, 22 * _uW, 2 * _uH];
    ...
    _ctrlSquadEntryCombo ctrlAddEventHandler ["LBSelChanged", {
        params ["_control", "_selectedIndex"];
        A3A_planning_selectedSquadEntry = _control lbData _selectedIndex;
    }];

    // HQ Garage Vehicle dropdown (only visible when Vehicle Crew Squad is selected)
    private _ctrlVehLabel = _display ctrlCreate ["TextBase", 8031, _controlsGroup];
    _ctrlVehLabel ctrlSetPosition [1 * _uW, 18.5 * _uH, 22 * _uW, 1.5 * _uH];
    _ctrlVehLabel ctrlSetText "Select Crew Vehicle (Garage):";
    ...
    _ctrlVehCombo ctrlSetPosition [1 * _uW, 20 * _uH, 22 * _uW, 2 * _uH];
    ...
    // Queue button (position finalized dynamically in the refresh section below)
    private _ctrlBtnQueue = _display ctrlCreate ["ButtonBase", 8023, _controlsGroup];
    _ctrlBtnQueue ctrlSetPosition [1 * _uW, 18.5 * _uH, 22 * _uW, 2 * _uH];
    ...
    _ctrlBtnQueue ctrlAddEventHandler ["ButtonClick", {
        ...
        private _garageVeh = "";
        if (A3A_planning_selectedSquadIndex == 8) then {
            private _vehCombo = _display displayCtrl 8030;
            _garageVeh = _vehCombo lbData (lbCurSel _vehCombo);
        };

        if (A3A_planning_selectedSquadIndex == 8 && {_garageVeh == ""}) exitWith {
            ["Denied", "You must select an available vehicle from the HQ Garage for the crew squad.", true] call A3A_fnc_planning_showNotification;
        };

        private _details = [A3A_planning_selectedSquadIndex, _garageVeh] call A3A_fnc_planning_getSquadDetails;
    ...
    _ctrlQueueList ctrlSetPosition [1 * _uW, 21 * _uH, 22 * _uW, 6 * _uH];
    ...
    _ctrlBtnRemove ctrlSetPosition [1 * _uW, 27.5 * _uH, 22 * _uW, 2 * _uH];
    ...
    _ctrlCapChk cbSetChecked true; // Default to checked

    private _ctrlCapLabel = _display ctrlCreate ["TextBase", 8043, _controlsGroup];
    _ctrlCapLabel ctrlSetPosition [3.5 * _uW, 32.5 * _uH, 19.5 * _uW, 2 * _uH];
    ...
    _ctrlCostLabel ctrlSetPosition [1 * _uW, 35 * _uH, 22 * _uW, 5.5 * _uH];
    ...
    _ctrlBtnCommence ctrlSetPosition [1 * _uW, 41 * _uH, 22 * _uW, 3 * _uH];
    ...
    _ctrlBtnClear ctrlSetPosition [1 * _uW, 44.5 * _uH, 22 * _uW, 2.5 * _uH];
    ...
    _ctrlBtnCommence ctrlAddEventHandler ["ButtonClick", {
        ...
        private _addHC = cbChecked (_display displayCtrl 8040);
        ...
        // Only clear the recruitment queue on commencement; planning markers and entry points persist until siege conclusion
        A3A_planning_queue = [];
    ...
    // Dummy control at the bottom of the controls group to force scrollable canvas height on low resolutions
    private _ctrlDummy = _display ctrlCreate ["TextBase", 8099, _controlsGroup];
    _ctrlDummy ctrlSetPosition [1 * _uW, 44.5 * _uH, 22 * _uW, 0.1 * _uH];
...
if (A3A_planning_objective != "") then {
    ...
    if (_name == "") then { _name = A3A_planning_objective; };
...
if (count A3A_planning_entryPoints > 0) then {
    ...
    if (_squadSel != -1) then { _ctrlSquadEntryCombo lbSetCurSel _squadSel; } else { _ctrlSquadEntryCombo lbSetCurSel 0; };
};

// Populate squad type listbox (if empty)
private _ctrlSquadCombo = _display displayCtrl 8020;
if (lbSize _ctrlSquadCombo == 0) then {
...
// Determine crew-selector visibility BEFORE laying out anything below it,
// so the whole panel can collapse/expand around it correctly.
private _selIndex = lbCurSel _ctrlSquadCombo;
private _dataId = if (_selIndex != -1) then { _ctrlSquadCombo lbData _selIndex } else { "-1" };
private _isCrewSel = (_dataId == "8");
private _ctrlVehLabel = _display displayCtrl 8031;
private _ctrlVehCombo = _display displayCtrl 8030;
_ctrlVehLabel ctrlShow _isCrewSel;
_ctrlVehCombo ctrlShow _isCrewSel;

// Populate available vehicles from HQ Garage
if (_isCrewSel) then {
    // 1. Gather all garage vehicles
    private _availableVehicles = +vehInGarage;

    // 2. Subtract vehicles already allocated in the queue
    {
        _x params ["_type", "_id", "_special", "_costMoney", "_costHR", "_veh", "_name", "_entry"];
        if (_special == "GarageCrew" && {_veh != ""}) then {
            private _idx = _availableVehicles find _veh;
            if (_idx != -1) then {
                _availableVehicles deleteAt _idx;
            };
        };
    } forEach A3A_planning_queue;

    // 3. Clear and add available unique vehicles to the dropdown
    lbClear _ctrlVehCombo;
    private _uniqueVehs = [];
    {
        if !(_x in _uniqueVehs) then { _uniqueVehs pushBack _x; };
    } forEach _availableVehicles;

    private _vIdx = 0;
    {
        private _className = _x;
        private _cnt = { _x == _className } count _availableVehicles;
        
        // Query persistent global cache for display names to optimize FPS
        private _displayName = A3A_planning_cachedDisplayNames get _className;
        if (isNil "_displayName") then {
            _displayName = getText (configFile >> "CfgVehicles" >> _className >> "displayName");
            if (_displayName == "") then { _displayName = _className; };
            A3A_planning_cachedDisplayNames set [_className, _displayName];
        };

        private _itemText = format ["%1 (%2 In Garage)", _displayName, _cnt];
        _ctrlVehCombo lbAdd _itemText;
        _ctrlVehCombo lbSetData [_vIdx, _className];
        _vIdx = _vIdx + 1;
    } forEach _uniqueVehs;

    if (lbSize _ctrlVehCombo > 0) then {
        _ctrlVehCombo lbSetCurSel 0;
    } else {
        _ctrlVehCombo lbAdd "No Vehicles Available";
        _ctrlVehCombo lbSetData [0, ""];
        _ctrlVehCombo lbSetCurSel 0;
    };
};

// --- DYNAMIC LAYOUT: collapse the gap left by the crew-vehicle selector when hidden ---
// Base (non-crew) Y positions; add this offset for every control below the
// staging dropdown whenever the vehicle selector is actually visible.
private _crewOffset = if (_isCrewSel) then { 4 } else { 0 };

private _ctrlBtnQueue = _display displayCtrl 8023;
_ctrlBtnQueue ctrlSetPosition [1 * _uW, (18.5 + _crewOffset) * _uH, 22 * _uW, 2 * _uH];
_ctrlBtnQueue ctrlCommit 0;

private _ctrlQueueList = _display displayCtrl 8014;
_ctrlQueueList ctrlSetPosition [1 * _uW, (21 + _crewOffset) * _uH, 22 * _uW, 6 * _uH];
_ctrlQueueList ctrlCommit 0;

private _ctrlBtnRemove = _display displayCtrl 8024;
_ctrlBtnRemove ctrlSetPosition [1 * _uW, (27.5 + _crewOffset) * _uH, 22 * _uW, 2 * _uH];
_ctrlBtnRemove ctrlCommit 0;

private _ctrlHCChk = _display displayCtrl 8040;
_ctrlHCChk ctrlSetPosition [1 * _uW, (30 + _crewOffset) * _uH, 2 * _uW, 2 * _uH];
_ctrlHCChk ctrlCommit 0;

private _ctrlHCLabel = _display displayCtrl 8041;
_ctrlHCLabel ctrlSetPosition [3.5 * _uW, (30 + _crewOffset) * _uH, 19.5 * _uW, 2 * _uH];
_ctrlHCLabel ctrlCommit 0;

private _ctrlCapChk = _display displayCtrl 8042;
_ctrlCapChk ctrlSetPosition [1 * _uW, (32.5 + _crewOffset) * _uH, 2 * _uW, 2 * _uH];
_ctrlCapChk ctrlCommit 0;

private _ctrlCapLabel = _display displayCtrl 8043;
_ctrlCapLabel ctrlSetPosition [3.5 * _uW, (32.5 + _crewOffset) * _uH, 19.5 * _uW, 2 * _uH];
_ctrlCapLabel ctrlCommit 0;

private _ctrlCostLabel = _display displayCtrl 8025;
_ctrlCostLabel ctrlSetPosition [1 * _uW, (35 + _crewOffset) * _uH, 22 * _uW, 5.5 * _uH];
_ctrlCostLabel ctrlCommit 0;

private _ctrlBtnCommence = _display displayCtrl 8026;
_ctrlBtnCommence ctrlSetPosition [1 * _uW, (41 + _crewOffset) * _uH, 22 * _uW, 3 * _uH];
_ctrlBtnCommence ctrlCommit 0;

private _ctrlBtnClear = _display displayCtrl 8027;
_ctrlBtnClear ctrlSetPosition [1 * _uW, (44.5 + _crewOffset) * _uH, 22 * _uW, 2.5 * _uH];
_ctrlBtnClear ctrlCommit 0;

private _ctrlDummy = _display displayCtrl 8099;
_ctrlDummy ctrlSetPosition [1 * _uW, (47 + _crewOffset) * _uH, 22 * _uW, 0.1 * _uH];
_ctrlDummy ctrlCommit 0;
...
_ctrlBtnEntryDel ctrlEnable (_hasObjective && {_hasStaging});
...
private _maxHcLimit = if (player call A3A_fnc_isMember) then { 10 } else { 6 };
...

{
    ...
    if (_special == "GarageCrew" && {_veh != ""}) then {
        private _vehName = A3A_planning_cachedDisplayNames get _veh;
        if (isNil "_vehName") then {
            _vehName = getText (configFile >> "CfgVehicles" >> _veh >> "displayName");
            if (_vehName == "") then { _vehName = _veh; };
            A3A_planning_cachedDisplayNames set [_veh, _vehName];
        };
        _label = format ["%1 [%2]", _name, _vehName];
    };
    _ctrlQueueList lbAdd (format ["%1 (%2 € / %3 HR) - Sp: %4", _label, _costMoney, _costHR, _entry]);
...
if (A3A_planning_assaultStarted) then {
    ...
    private _ctrlBtnEntryMove = _display displayCtrl 8017;
    private _ctrlBtnEntryDel = _display displayCtrl 8018;
    if (!isNull _ctrlBtnEntryAdd) then { _ctrlBtnEntryAdd ctrlEnable false; _ctrlBtnEntryAdd ctrlSetTooltip "An assault is currently in progress."; };
    if (!isNull _ctrlBtnEntryMove) then { _ctrlBtnEntryMove ctrlEnable false; _ctrlBtnEntryMove ctrlSetTooltip "An assault is currently in progress."; };
    if (!isNull _ctrlBtnEntryDel) then { _ctrlBtnEntryDel ctrlEnable false; _ctrlBtnEntryDel ctrlSetTooltip "An assault is currently in progress."; };
    ...
    if (!isNull _ctrlSquadCombo) then { _ctrlSquadCombo ctrlEnable false; };
    if (!isNull _ctrlSquadEntryCombo) then { _ctrlSquadEntryCombo ctrlEnable false; };
    if (!isNull _ctrlVehCombo) then { _ctrlVehCombo ctrlEnable false; };
    if (!isNull _ctrlBtnQueue) then { _ctrlBtnQueue ctrlEnable false; };
    if (!isNull _ctrlQueueList) then { _ctrlQueueList ctrlEnable false; };
    if (!isNull _ctrlBtnRemove) then { _ctrlBtnRemove ctrlEnable false; };
    if (!isNull _ctrlHCChk) then { _ctrlHCChk ctrlEnable false; };
    if (!isNull _ctrlCapChk) then { _ctrlCapChk ctrlEnable false; };
    if (!isNull _ctrlBtnCommence) then { _ctrlBtnCommence ctrlEnable false; _ctrlBtnCommence ctrlSetTooltip "An assault is currently in progress."; };
    if (!isNull _ctrlBtnClear) then { _ctrlBtnClear ctrlEnable false; _ctrlBtnClear ctrlSetTooltip "An assault is currently in progress."; };
};
...
_controlsGroup ctrlSetPosition [_currentPos # 0, _currentPos # 1, _currentPos # 2, _currentPos # 3];
_controlsGroup ctrlCommit 0;
